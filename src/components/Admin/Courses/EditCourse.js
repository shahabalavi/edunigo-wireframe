...
const EditCourse = () => {
  ...
  // Load data and course information
  useEffect(() => {
    // Simulate API calls
    setTimeout(() => {
      ...
      // Sample course data (in real app, this would come from API based on ID)
      const sampleCourse = {
        id: parseInt(id),
      ...
      setFormData({
        ...
        educationLevelId: sampleCourse.educationLevel.id.toString(),
        majorId: sampleCourse.major.id.toString(),
        universityIds: sampleCourse.universities.map((uni) => uni.id),
        startDate: sampleCourse.startDate,
        endDate: sampleCourse.endDate,
      });
      setLoading(false);
    }, 1000);
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Updating course:", submitData);
